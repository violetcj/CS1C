...
int main()
{
    ...
    cout << "*      CS1C 7/8/21 3:00-5:20         * " << endl;
    cout << "************************************** " << endl;
    cout << endl;
    ...
    // NOT ALLOWED cannot have a type pointer to double point to an int
    *p = 5; // [1.3] update the value of p to 5, use * to assign to x through p
    ...
    //p2 is still pointing to x, as its address has never changed
    ...
    //type reference to double cannot be initialized with a type int value // [1.9] variable s of type reference to double refers to y
    ...
    *pd = 3;
    ...
    //pd += 1000;    // [2.3] // pd points to an address outside of the array's memory, creates a read access violation; error explain
    //double d = *pd;// [2.4] // It works to a degree, but they are stored in different memory; error explain
    //*pd = 12.34;   // [2.5] // this one doesnt provide any errors to me. error explain

    cout << pd2 - pd; // [2.6] compute the number of elements between pd and pd2 by taking the difference between pd2 and pd
    //int dif2 = pd + pd2; // [2.7] // expression must have integral or unscoped enum type
    //int dif3 = pd2 - pd3;// [2.8] // these two pointers each point to a different array
    ...
    //pd6 and pd7 are pointing to different arrays
    ...
    // Q#4 - new, delete operator examples
    {
        ...
        //pi = pd;  // [4.5] pi points to pd
                    // pd is of type double while pi is of type int error explain
        //pd = pi;  // [4.6] pd points to pi
                    // cannot assign pi of type int to pd of type double error explain
